export function createParticleSystem(scene, count = 1500) {
  ...
  const rgb = [[0.96,0.94,0.98],[1,0.62,0.8],[1,0.88,0.35],[0.42,0.92,0.65],[0.38,0.82,1],[0.82,0.48,1],[1,0.52,0.32]];

  for (let i = 0; i < count; i++) {
    const i3 = i * 3, r = 2.0 + Math.sqrt(Math.random()) * 92.0, th = Math.random() * 6.28318;
  ...
  const mat = new THREE.ShaderMaterial({
    vertexShader: `attribute vec3 customColor;attribute vec2 aSeed;uniform float uTime,uAwakened;varying vec3 vColor;varying float vBlade;void main(){vColor=mix(vec3(0.7),customColor,uAwakened);vec3 p=position;float t=uTime*aSeed.y+aSeed.x,g=sin(p.x*0.06+p.z*0.05+uTime*2.2)*1.8,w=sin(p.x*0.12+p.z*0.09+t*2.8);p.xz+=vec2(sin(t+p.z*0.04)*2.0+w*1.5+g,cos(t+p.x*0.04)*1.8+w+g*0.7);p.y+=sin(t*2.0+p.x*0.2)*0.7;vBlade=step(0.5,fract(aSeed.x*3.7));vec4 mv=modelViewMatrix*vec4(p,1.0);gl_PointSize=clamp((72.0/-mv.z)*(2.2+uAwakened*1.5)*(vBlade>0.5?1.4:1.0),4.0,56.0);gl_Position=projectionMatrix*mv;}`,
    fragmentShader: `precision highp float;varying vec3 vColor;varying float vBlade;void main(){vec2 uv=gl_PointCoord-vec2(0.5);float d=length(uv);if(d>0.5)discard;float c=pow(1.0-d*2.0,1.8);gl_FragColor=vec4(mix(vColor,vec3(1.0),c*0.65)*(1.4+c),c*0.95);}`,
    uniforms,
    transparent: true,
