...
export function createFogCards(scene, count = 36) {
  ...
  const mat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `precision highp float;uniform float uTime,uAwakened,uFogDensity;varying vec2 vUv;varying vec3 vWP,vVP;void main(){float d=length(vVP),pF=smoothstep(5.0,15.0,d),hF=1.0-smoothstep(150.0,240.0,d);float bFade=smoothstep(0.0,0.48,vUv.y),tFade=smoothstep(1.0,0.60,vUv.y),edge=pow(sin(vUv.x*3.1416),1.4)*bFade*tFade;float t=uTime*0.25,wisp=sin(vWP.x*0.06+vWP.z*0.05+t*0.8)*cos(vWP.z*0.08-vWP.x*0.04-t*0.6)*0.5+0.5;vec3 col=mix(vec3(0.55,0.58,0.68),vec3(0.72,0.64,0.78)+vec3(0.04,0.04,0.08)*sin(t+vWP.x*0.1),uAwakened);gl_FragColor=vec4(col,edge*(0.20+wisp*0.24)*pF*hF*uFogDensity);}`,
    uniforms,
    transparent: true,
    depthTest: true,
    depthWrite: false,
  ...
  for (let i = 0; i < count; i++) {
    ...
    const w = isInner ? 16 + Math.random() * 10 : 38 + Math.random() * 18;
    const h = isInner ? 6 + Math.random() * 4 : 16 + Math.random() * 8;
